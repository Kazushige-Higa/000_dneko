...
#認証キーワードの代わりに使う簡易スパム対策
$conf['honeypot_field'] = 'website_url';
$conf['started_at_field'] = 'form_started_at';
...
#認証用ドメイン ※カンマ区切り(,)で作成サーバ(例：ros-serv031.oops.jp)の本番用ドメイン(例：xxxx.com等)を設定してください。(設定例：ros-serv031.oops.jp,xxxx.com)
$conf['domain'] = "";

#0の場合は全て表示。1の場合は入力していない項目はメールに記載されません。
$conf['no_disp'] = '0';

#問い合わせ先情報(trueの場合、管理者にはIP、サーバ情報などを送る。送りたくない場合はfalseにする。)
$conf['admin_posted'] = true;

#送信者名
$conf['name'] = 'デザネコ';

#設置者のアドレス(カンマ区切り) ※先頭のアドレスがFromとして扱われる
...
#送信者に届くメールの件名
$conf['res_subject'] = 'デザネコへのお問い合わせありがとうございます。';

#送信者に届くメールの本文
$conf['res_body'] = <<< EOM
この度は、デザネコへお問い合わせいただきありがとうございます。
内容を確認のうえ、担当者よりご連絡いたしますので
今しばらくお待ちください。
...
if (isset($_POST['form_type']) && in_array($_POST['form_type'], ['contact', 'marutto_contact'], true)) {
    ...
    $conf['res_subject'] = 'デザネコへのお問い合わせを受け付けました。';
    $conf['res_body'] = <<< EOM
この度は、デザネコへお問い合わせいただきありがとうございます。
内容を確認のうえ、担当者よりご連絡いたしますので
今しばらくお待ちください。

万が一3日以内に返信がない場合は、メールアドレスの入力間違いなどの可能性があります。
お手数ですが、再度お問い合わせいただくか公式LINEよりご連絡ください。
...
if (isset($_POST['form_type']) && 'oota_tour' === $_POST['form_type']) {
    $conf['subject'] = '【参加申込】南の島のおばーの夢叶えるバスツアー';
    $conf['res_subject'] = '【参加お申込みありがとうございます】夢叶える国頭バスツアー';
    $conf['res_body'] = <<< EOM
参加お申込みありがとうございます。
メール確認いたしました。
担当者よりご連絡さしあげますのでしばらくお待ちください。

＊お申込みいただいてから3日経過しても、担当者より連絡がない場合は、
　お手数ですが、お電話にてお問合せをお願いいたします。
...
━━━━━━━━━━━━━━━━━━━━━━━━━━
主催：おばーの夢叶え隊プロジェクト 代表：大田浩之
■運営協力：悠久の旅沖縄 アニー梅本 098-943-3385
Mail  info@yukyu-okinawa.com
━━━━━━━━━━━━━━━━━━━━━━━━━━
EOM;
}

#添付可能ファイル拡張子
$conf['file_exp'] = 'jpg,jpeg,png,gif,pdf,jww,xlsx,xlsm,xls,doc,docx,ai,zip,lzh';
...
/**
 * 送信タイプがsmtpかつホスト名、ユーザ名、パスワード、セキュリティ、ポート番号に
 * 値が入っている場合にSMTP送信を行う。
 * SMTP送信を行わない場合は送信タイプは空白にする。
 */
#送信タイプ
$conf['send_type'] = '';
...
class PHPMailer
{
    ...
    public function preSend()
    {
        ...
        try {
            ...
            if (is_array_count($this->to) + is_array_count($this->cc) + is_array_count($this->bcc) < 1) {
    ...
    protected function mailSend($header, $body)
    {
        ...
        $result = false;
        if ($this->SingleTo and is_array_count($toArr) > 1) {
    ...
    protected function smtpSend($header, $body)
    {
        ...
        if ((is_array_count($this->all_recipients) > is_array_count($bad_rcpt)) and !$this->smtp->data($header . $body)) {
    ...
    public function createHeader()
    {
        ...
        if ($this->SingleTo) {
            ...
            } elseif (is_array_count($this->cc) == 0) {
        ...
        if ((
            ...
            and is_array_count($this->bcc) > 0
        ) {
            $result .= $this->addrAppend('Bcc', $this->bcc);
        }

        if (is_array_count($this->ReplyTo) > 0) {
    ...
    {
        if (is_array_count($this->language) < 1) {
        ...
        if (array_key_exists(2, $images)) {
            ...
            foreach ($images[2] as $imgindex => $url) {
                ...
                if (preg_match('#^data:(image/(?:jpe?g|gif|png));?(base64)?,(.+)#', $url, $match)) {
                    if (is_array_count($match) == 4 and 'base64' == $match[2]) {
...
class SMTP
{
    ...
    public function connect($host, $port = null, $timeout = 30, $options = [])
    {
        ...
        $this->edebug(
            ...
            (is_array_count($options) > 0 ? var_export($options, true) : 'array()'),
    ...
    protected function sendCommand($command, $commandstring, $expect)
    {
        ...
        if (preg_match('/^([0-9]{3})[ -](?:([0-9]\\.[0-9]\\.[0-9]) )?/', $this->last_reply, $matches)) {
            $code = $matches[1];
            $code_ex = (is_array_count($matches) > 2 ? $matches[2] : null);
...
/**
 * メールフォームチェック処理を行うための情報を画面へ出力する。
 * メールを送信するために$confへ格納されている情報をjson形式で画面出力する。
 */
function mail_from_check() {
	global $conf;

	// json形式で画面出力
	echo create_json($conf);
}
...
function send_mail() {
	...
	if(0 < is_array_count($_POST)) {
		...
		$honeypot_filled = isset($_POST[$honeypot_field])
			&& (!is_string($_POST[$honeypot_field]) || "" !== trim($_POST[$honeypot_field]));
		$started_at = isset($_POST[$started_at_field]) && is_scalar($_POST[$started_at_field])
			? (int)$_POST[$started_at_field]
			: 0;
		$elapsed_seconds = ($started_at > 0) ? (time() - $started_at) : -1;

		if($honeypot_filled) {
			header('Location: ' . $safe_failure_redirect, true, 303);
			exit;
		}
		if(
			$started_at <= 0 ||
			$elapsed_seconds < (int)$conf['min_submit_seconds'] ||
			$elapsed_seconds > (int)$conf['max_submit_seconds']
		) {
			header('Location: ' . $safe_failure_redirect, true, 303);
			exit;
		}
        // 対象のページからのpostかチェックする
        if(null != $conf['domain'] && "" != $conf['domain']) {
            $domains = explode(',',$conf['domain']);
            $post_url = parse_url($_SERVER["HTTP_REFERER"]);
            $not_target_post_flag = true;
            // 対象のページからのpostか判定
            foreach($domains as $domain) {
                if(stristr($post_url['host'], $domain)) {
                    $not_target_post_flag = false;
                }
            }
            // 対象のページ以外からのpostの場合
            if ($not_target_post_flag) {
                header("Location: ".$_SERVER["HTTP_REFERER"]);
                exit;
            }
        }

        if ($is_marutto_contact_request) {
            $invalid_redirect = $marutto_invalid_redirect;
            ...
            $read_post_string = static function ($key, $default = '') use (&$has_non_scalar_field) {
                if (!isset($_POST[$key])) {
                    return $default;
                }
            ...
            $consultations = isset($_POST['ご相談内容(必須)']) && is_array($_POST['ご相談内容(必須)'])
                ? array_values($_POST['ご相談内容(必須)'])
                : [];
            $name = $read_post_string('お名前(必須)');
            $company = $read_post_string('屋号・会社名');
            $email = $read_post_string('email(必須)');
            $message = $read_post_string('お問い合わせ内容(必須)');
            $website = $read_post_string('今お持ちのホームページURL');
            $phone = $read_post_string('電話番号');
            $reply_method = $read_post_string('ご希望の連絡方法', 'メール');
            $budget = $read_post_string('ご予算感');
            $posted_token = $read_post_string('marutto_form_token');
            $session_token = isset($_SESSION['marutto_form_token']) ? (string)$_SESSION['marutto_form_token'] : '';
            $last_submit_at = isset($_SESSION['marutto_last_submit_at']) ? (int)$_SESSION['marutto_last_submit_at'] : 0;
            ...
            $has_invalid_token = '' === $posted_token
                || '' === $session_token
                || !hash_equals($session_token, $posted_token);
            $is_rate_limited = $last_submit_at > 0 && (time() - $last_submit_at) < 60;

            if ($has_unknown_field || $has_non_scalar_field || $has_invalid_consultation || $has_invalid_required_value || $has_invalid_optional_value || $has_invalid_token || $is_rate_limited) {
		...
		foreach($_POST as $key =>  $val) {
			...
			if('0' == $conf['no_disp'] || "" != $val) {
				...
				if(
					...
					if(is_array($val) && 0 < is_array_count($val)) {

						foreach((array)$val as $x) {
							$cons .= (is_array($val) && 1 < is_array_count($val))?$x.'、':$x;
						}

					} else {
						$cons = $val;
					}
					$con = mb_convert_encoding($cons, "UTF-8");
					$con = mb_convert_kana($con, "KVa");

					//POST配列用[]削除
					$key_title = str_replace("[]", "", $key);

					$resbody .= '[ '.$key_title." ] ". $con ."\n";
				}
			}
		}
		...
		if(0 < is_array_count($_FILES['file']['name'])) {
			$files = check_file();
        ...
        $admin_posted_body = "";
        if (true === $conf['admin_posted']) {
            // 管理者に送る問い合わせ情報
            $admin_posted_body .= "\n\n";
            $admin_posted_body .= "[ HOST NAME ] ".gethostbyaddr($_SERVER['REMOTE_ADDR'])."\n";
            $admin_posted_body .= "[ IP ADDRESS ] ".$_SERVER["REMOTE_ADDR"]."\n";
            $admin_posted_body .= "[ USER AGENT ] ".$_SERVER["HTTP_USER_AGENT"]."\n";
            $admin_posted_body .= "[ HTTP REFERER ] ".$_SERVER["HTTP_REFERER"]."";
        }
        // 管理者用(メール本文)
		...
		//-----------------------------------添付ファイル処理開始----------------------------------
		// 正常な添付ファイルが存在する場合
		if (0 < is_array_count($files)) {
			//**************************
			//ファイル一時格納フォルダ作成
			//**************************
			$tmp_folder_nm = "file";
			$tmp_file_dir = "./".$tmp_folder_nm."/";
			if(!file_exists($tmp_file_dir)) {
				// 存在しないときfileフォルダを作成(umaskの影響で属性値が「755」にならない)
				mkdir($tmp_file_dir, 0755);
				// 気合で755にする
				chmod($tmp_folder_nm, 0755);
			}
			
			foreach($files as $key) {

				$file_name = mb_convert_encoding($_FILES['file']['name'][$key],'UTF-8');
				
				if("" != $file_name) {

					$fullname = $tmp_file_dir.$file_name;
                    move_uploaded_file($_FILES['file']['tmp_name'][$key],$fullname);
                    // ファイルパス格納
                    $attachment_files[] = $fullname;
		...
		// カンマ区切りの複数メール対策
		$mailtos = explode(',',$conf['mailto']);

		// 送信者メールアドレス取得(「email」または「email(必須)」フィールドに対応)
		$user_email = isset($_POST['email']) ? trim((string)$_POST['email']) : '';
		if ('' === $user_email && isset($_POST['email(必須)'])) {
			$user_email = trim((string)$_POST['email(必須)']);
		}

        //**************************
        // 管理者へメール送信
        //**************************
        $admin_mailer = new PHPMailer(true);
        $admin_mailer->CharSet = $conf['char']; // 文字コード設定
		$admin_mailer->Encoding = $conf['encoding']; // 文字エンコード設定
        
        // SMTP送信する場合
        if ("SMTP" == strtoupper($conf['send_type'])) {

            $smtp_host = $conf['smtp_host']; // ホスト名
            $smtp_username = $conf['smtp_username']; // ユーザ名
            $smtp_password = $conf['smtp_password']; // パスワード
            $smtp_secure = $conf['smtp_secure']; // セキュリティ(SSLやTLSなど)
            $smtp_port = $conf['smtp_port']; // ポート番号(465や587など)

            // ホスト名、ユーザ名、パスワード、セキュリティ、ポート番号がすべて設定されている場合にSMTP送信する
            if ((null != $smtp_host && "" != $smtp_host) && (null != $smtp_username && "" != $smtp_username)
                    && (null != $smtp_password && "" != $smtp_password) && (null != $smtp_secure && "" != $smtp_secure) && (null != $smtp_port && "" != $smtp_port) 
            ) {
                $admin_mailer->isSMTP();
                $admin_mailer->Host = $smtp_host;
                $admin_mailer->SMTPAuth = true;
                $admin_mailer->Username = $smtp_username;
                $admin_mailer->Password = $smtp_password;
                $admin_mailer->SMTPSecure = $smtp_secure;
                $admin_mailer->Port = $smtp_port;
            }
		...
		foreach($mailtos as $mailto) {
			$admin_mailer->AddAddress($mailto);
		}
		// 問合せフォームのメールが設定されている場合、返信先メールアドレスをお客さんのメールにする
		if ("" != $user_email) {
        ...
        foreach($attachment_files as $attachment_file) {
			$admin_mailer->AddAttachment($attachment_file);
        }
        // 管理者へメール送信
        $admin_mailer->Send();
        
        //*********************************************
        // 問い合わせしたユーザへメール送信(自動返信メール)
        //*********************************************
        if("" != $user_email) {
            $user_mailer = new PHPMailer(true);
            $user_mailer->CharSet = $conf['char']; // 文字コード設定
            $user_mailer->Encoding = $conf['encoding']; // 文字エンコード設定
            ...
            if ("SMTP" == strtoupper($conf['send_type'])) {
                ...
                if ((null != $smtp_host && "" != $smtp_host) && (null != $smtp_username && "" != $smtp_username)
                    ...
                    $user_mailer->isSMTP();
                    $user_mailer->Host = $smtp_host;
                    $user_mailer->SMTPAuth = true;
                    $user_mailer->Username = $smtp_username;
                    $user_mailer->Password = $smtp_password;
                    $user_mailer->SMTPSecure = $smtp_secure;
                    $user_mailer->Port = $smtp_port;
                }
            }
            ...
            // ファイル添付
            foreach($attachment_files as $attachment_file) {
                $user_mailer->AddAttachment($attachment_file);
            }
            // 問い合わせユーザへメール送信
            $user_mailer->Send();
        }
...
/**
 * 添付ファイルのチェックを行う。
 * エラーが存在する場合はエラー情報にメッセージを設定する。
 * チェックした結果エラーがないファイルの要素番号のみを返却する。
 */
function check_file() {
	global $conf,$err;
	...
	// 添付可能ファイル拡張子をリスト形式で取得
	$exps = explode(',',$conf['file_exp']);

	foreach($_FILES['file']['name'] as $key => $val) {
			
		$file_name = mb_convert_encoding($_FILES['file']['name'][$key],'UTF-8','auto');
		
		if("" != $file_name) {
			
			//拡張子取得
			$exp = mb_strtolower(end(explode('.', $file_name)));
			...
			if(in_array($exp, $exps)) {
				...
				$err["msg"] .= "\n「".$file_name."」というファイルを添付しようとしましたが失敗しました。";
			}
		}
	}
	
	if ("" != $err["msg"]) {
		$err["msg"] .= "\nファイルは「".$conf['file_exp']."」のみ添付可能です。";
	}
...
/**
 * json形式にする。
 * データリストをjson形式にして返却する。
 * @param $data_list json形式するのデータリスト
 * @return json形式の情報
 */
function create_json($data_list) {

	$view_str = "";
	foreach ($data_list as $key => $data_value) {
		if ("" == $view_str) {
			$view_str .= "{";
		} else {
			$view_str .= ",";
		...
		if (is_array($data_value)) {
			if (empty($data_value)) {
				...
				$view_str .= "\"".$key."\"".":";
				$view_str .= create_json($data_value);
			}
		} else {
			$view_str .= "\"".$key."\"".":"."\"".$data_value."\"";
		}
	}
	$view_str .= "}";
...
function is_array_count(?array $array):int {
    $cnt = 0;
